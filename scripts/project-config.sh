...
PROJECT_NAME="EventRunner6"

# GitHub repository (format: owner/repo)
GITHUB_REPO="jangabrielsson/EventRunner6"

# Forum thread URL (optional - leave empty if not applicable)
FORUM_URL="https://forum.fibaro.com/topic/79165-eventrunner-6/"
...
declare -a VERSION_FILES=(
    "src/rule.lua:^local VERSION"
    "src/eventrunner.lua:^local VERSION"
    "src/updater.lua:^local VERSION"
)
...
declare -a ARTIFACTS=(
    "src/eventrunner.lua:dist/EventRunner6.fqa:plua -t pack {SOURCE} {OUTPUT}"
    "src/updater.lua:dist/ERUpdater.fqa:plua -t pack {SOURCE} {OUTPUT}"
)
